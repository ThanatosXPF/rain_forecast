...
class CnnNetwork():
    ...
    def scale(self, X: np.ndarray, mode="normal"):
        ...
        # scale_3 = []
        # for x in X:
        scale_1 = X[:, 15: -15, 15: -15, :]
        scale_2 = X[:, 19: -19, 19: -19, :]
        scale_3 = X[:, 25: -25, 25: -25, :]
        # scale_1 = X.reshape([batch_size, shape[1], shape[2], 10])
        scale_1 = np.asarray(scale_1).reshape([batch_size, 31, 31, 10])
        scale_2 = np.asarray(scale_2).reshape([batch_size, 23, 23, 10])
    ...
    def train_step(self, X, y):
        scale_1, scale_2, scale_3 = self.scale(X)
        y_t_2 = y.copy()
        y_t_2[y_t_2 == 0] = 0.5
        _, loss, bias, error = self.sess.run([self.optimizer, self.loss, self.bias, self.error_rate],
                                             feed_dict={self.input_frame: scale_1,
...
def do_prediction(start, end):
    runner = CnnNetwork(300, "/extend/rain_data/Save/Model/resize_31/model.ckpt-43",
                        mode="test")
    print("loaded!")
...
def building_data():
    path = '/home/ices/work/image-t-rain/new/data'
    ...
    for file in files:
        if file[-4:] == '.npy':
            # print(file)
            X.append(np.load(path + "/" + file))
        else:
            Y = np.loadtxt(path + "/" + file, delimiter=',').reshape(-1, 1)
...
def do_test():
    ...
    for i in range(300):
        ...
        t = 0
        while True:
            X_b = X_train[p: p+c.CNN_BATCH_SIZE,20:-20,20:-20,:]
            y_b = Y_train[p: p+c.CNN_BATCH_SIZE]
            p += c.CNN_BATCH_SIZE
            if 4500 - p <= c.CNN_BATCH_SIZE:
                p = 0
                break
            loss, bias,error = runner.train_step(X_b, y_b)
            t_loss += loss
            t_bias += bias
            t_error += error
            t += 1
        ...
        print(t_bias/t)
        print(t_error/t)
        do_valid(runner, X_test, Y_test)

def do_valid(runner, X_test, y_test):

    X_b = X_test[:c.CNN_BATCH_SIZE, 20:-20, 20:-20, :]
    y_b = y_test[:c.CNN_BATCH_SIZE]
    loss, bias, error = runner.valid_step(X_b, y_b)
    print("TEST:", "------------------------")
    print(loss)
    print(bias)
    print(error)
